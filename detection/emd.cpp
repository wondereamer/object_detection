...
float emd_wapper(const Features &supplier, const std::vector<float> sWeight,
        ...
        float (*dist_fun)(feature_t *F1, feature_t *F2), VizBlockWorld *viz, float radius){
    assert(supplier.size() == sWeight.size() &&
            demand.size() == dWeight.size());
    int sSize = supplier.size();
    int dSize = demand.size();
    feature_t *f1 = new feature_t[supplier.size()];
    feature_t *f2 = new feature_t[demand.size()];
    float *wSource = new float[sWeight.size()];
    float *wTarget = new float[dWeight.size()];
    ...
    signature_t s1 = { supplier.size(), f1, wSource},
                s2 = { demand.size(), f2, wTarget};
    float       e;
    int  flowSize;
    e = emd(&s1, &s2, dist_fun, flow, &flowSize);
    if (viz) {
        for(auto &p : supplier){
            viz->add_sphere(p.x, p.y, p.z, radius, 0, 0, 255);
            /// @todo add 3d text
        ...
        for(auto &p : demand){
            viz->add_sphere(p.x, p.y, p.z, radius, 255, 0, 0);
        }
        for (int i=0; i < supplier.size() + demand.size() - 1; i++){
            feature_t from = supplier[flow[i].from];
            feature_t to = demand[flow[i].to];
            // mark source
            viz->add_text3D(m_util::sth2string<float>(wSource[flow[i].from]) ,from.x,
                    from.y, from.z, 0, 255,0, 0.05);
            // mark target
            viz->add_text3D(m_util::sth2string<float>(wTarget[flow[i].to]) , to.x,
                    to.y, to.z, 0,255,0, 0.05);
            if (flow[i].amount > 0){
                // mark flow
                viz->add_line(from.x, from.y, from.z, to.x, to.y, to.z);
                viz->add_text3D(m_util::sth2string<float>(flow[i].amount) ,(from.x + to.x)/2,
                        (from.y + to.y)/2, (from.z + to.z)/2, 255,0,0, 0.05);
            }
        }
    }
